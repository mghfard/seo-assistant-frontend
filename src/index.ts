        ...
        if (request.method === 'POST') {
            try {
                ...
                const findInBrief = (keys: string[]) => {
                    ...
                    for (let i = 0; i < body.brief.headers.length; i++) {
                        ...
                        if (header && typeof header === 'string') {
                            ...
                            for (const key of keys) { if (lowerHeader.includes(key)) { return body.brief.rowData[i]; } }
                        }
                    }
                    return null;
                };
                ...
                // --- وظیفه ۴: تولید مقاله کامل ---
                    ...
                    if (body.brief && body.brief.rowData) { for (const data of body.brief.rowData) { if (data && typeof data === 'string' && (data.startsWith('http'))) { sourceUrl = data; break; } } }
                    let articlePrompt = '';
                    if (sourceUrl) {
                        const response = await fetch(sourceUrl);
                        const htmlContent = await response.text();
                        const oldArticleText = stripHtml(htmlContent);
                        articlePrompt = `You are an expert SEO content rewriter. Your task is to update and refresh an existing article in PERSIAN. CONTEXT: 1. Final Approved Title: "${final_title}" 2. Content Brief: \n${structuredBrief} 3. New Outline to Follow: \n${outline} 4. IMPORTANT: The text of the OLD article is below:\n---\n${oldArticleText.substring(0, 8000)}\n--- CRITICAL INSTRUCTIONS: - Write a NEW and IMPROVED version of the article, avoiding duplicate content from the old text. - The Introduction MUST be between 100-150 words. - After each H2 heading, you MUST write an introductory paragraph (50-100 words) about that topic before starting its H3 subheadings. This is a mandatory rule. - The final article MUST BE AT LEAST ${word_count} words. - You MUST follow the new outline precisely. - Write in fluent, engaging Persian. - Return ONLY the full text of the article in Markdown.`;
                    } else {
                        articlePrompt = `You are a professional SEO content writer. Your task is to write a complete, comprehensive, and engaging blog post in PERSIAN. ALL INFORMATION PROVIDED: 1. Final Approved Title: "${final_title}" 2. Content Brief: \n${structuredBrief} 3. The exact Outline to follow: \n${outline} CRITICAL INSTRUCTIONS: - The Introduction MUST be between 100-150 words. - After each H2 heading, you MUST write an introductory paragraph (50-100 words) about that topic before starting its H3 subheadings. This is a mandatory rule. - YOU MUST WRITE AN ARTICLE OF AT LEAST ${word_count} WORDS. - You MUST follow the provided outline precisely. - Write in fluent, natural, and engaging Persian with short paragraphs. - Return ONLY the full text of the article in Markdown format.`;
                    }
                    ...
                    throw new Error("وظیفه نامشخص است. (نسخه نهایی)");
                }
            } catch (error: any) {
